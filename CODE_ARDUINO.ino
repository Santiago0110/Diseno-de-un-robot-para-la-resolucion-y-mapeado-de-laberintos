/*------------------------------------------------------------
 * ALGORITMO PARA RESOLUCIÓN DE LABERINTO
 * -----------------------------------------------------------
 * NOTA COMUNICACIÓN:
 * Se utilizan pines virtuales para comunicación serial 
 * (TX=10 , RX=11)
 * -----------------------------------------------------------
 * NOTA DISTANCIA: 
 ...
 * -----------------------------------------------------------
 * NOTA SERVOMOTOR:
 * Se utiliza el pin 12 para controlar el servo
 * -----------------------------------------------------------
 */
...
//Objeto llamado "Sensores"(TX,RX) para com. serial
...
int pos=0,i=0; 
float promedio=0;
int cont,jugar=0,anterior=0,elena=0;
...
int der_1=A0,der_2=A1;
int izq_1=A2,izq_2=A3;
int d1,d2,i1,i2;
...
void TomaDecision(){
  if(der==0 && frente==0 && izq==0 && anterior==0){
    ...
    Derecha();
  }else if(der==1 && frente==1 && izq==0 && anterior==1 && elena==1){      
    Derecha();
  }else if(der==1 && frente==1 && izq==0 && anterior==1 && elena==0){      
    Adelante();
